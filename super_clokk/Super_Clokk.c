...
{	
	...
	*	Damit nicht unn�tige Funktionsaufrufe von rtcBcdToDec() entstehen beim vergleichen
	*/
	...
	*/	
	...
	/*
	*	Aktuelle Uhrzeit mit den eingetellten Alarmzeiten �berpr�fen
	*/
...
{     
    ...
    /*
    *      Werte m�ssen beim Funktionsaufruf mitgegeben werden..
    *      Anhand der ersten beiden Bytes von "buff" werden die
    *      maximal Gr��en von den Werten festgelegt
    */
...
{			
	...
	*	Bit		Beschreibung
	*	
	*	0	-	Bei welchem Scan wurde ein Timeout ausgel�st [BIT0]
	*	1	-	Bei welchen Scan wurde ein Timeout ausgel�st [BIT1]
	*	2	-	Bei welchem Scan wurde ein Timeout ausgel�st [BIT2]
	...
	*/
	...
	for ( uint8_t y = 0 ; y < i && i > 1 ; y++ )
	{
		...
		{
			/*
			*	DCF77_RECORD bestimmt den sp�teren Wert von
			*	'cmpCnt'
			*/
			cmpCnt++;
		}
	...
	/*
	*	Sollte w�hrend der Funktion eine Taste bet�tigt wurden sein,
	*	m�ssen wir diese vor verlassen noch best�tigen
	*/
 	button.all = 0;
...
{
	/*	EEPROM
	*	Pr�fen ob der EEPROM schon initalisiert wurden ist..
	*/
	eeprom_busy_wait();
...
Anfang:
	...
	uint8_t buff[] = { 23 , 59 , eeprom_read_byte( &eep.byte8[ALERT_HOUR_1 + ret] ) , eeprom_read_byte( &eep.byte8[ALERT_MINUTE_1 + ret] ) }; // Darf nicht ver�ndert werden..
...
Anfang:
	...
	uint8_t buff[] = { 23 , 59 , eeprom_read_byte( &eep.byte8[ DIMM_HOUR_ON + ret ]) , eeprom_read_byte( &eep.byte8[ DIMM_MINUTE_ON + ret ]) }; // Darf nicht ver�ndert werden..
...
uint8_t menueAutoChange		( void )
{
	...
	uint8_t buff[] = { 59 , 59 , eeprom_read_byte( &eep.byte8[AUTO_CHANGE_MINUTE] ) , eeprom_read_byte( &eep.byte8[AUTO_CHANGE_SECOUND] ) }; // Darf nicht ver�ndert werden..
...
{		
	/*	DCF77 Abgleich
	*	Mit der Variable werden die verstrichenen Minuten gez�hlt 
	*	bis zur n�chsten Synchronisation
	*/
	...
	*/
	...
	/*	Eing�nge / Ausg�nge
	*	Anfangszust�nde der Pins festlegen
	*/
	PORTC |= ((1<<PC2) | (1<<PC3) | (1<<PC4) | (1<<PC5) | (1<<PC6));
	PORTB |= (1<<PB2);
	PORTD |= ((1<<PD5));
	
	/*	Eing�nge des Matrix-Treibers festlegen
	*/
	...
	*/
	...
	/*	Tasten Entprellen
	*	Sollte beim Systemstart eine Taste gedr�ckt wurden sein,
	*	l�schen wir diese nach main()
	*/
	button.all = 0;
	...
	*/
    ...
    while( 1 )
    {
		...
		/*	Zeitsynchronisation
		*	Kann im "Men�" -> "Sync?" -> "-Cycl." eingestellt werden
		*	...
		*/			
		...
		/*
		*	Z�hler f�r die automatische Synchronisation
		*/
